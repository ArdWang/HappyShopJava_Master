...
public class YuanFenConverter {
    ...
    public static String changeF2Y100(Long amounts) throws Exception {
        if (null == amounts) {
            return "0";
        }

        if (!amounts.toString().matches("\\-?[0-9]+")) {
    ...
    public static String changeF2Y100(String amount) throws Exception {
        if (!amount.matches("\\-?[0-9]+")) {
            throw new Exception("Invalid format");
        }
        return BigDecimal.valueOf(Long.valueOf(amount).longValue()*100).divide(new BigDecimal(100)).toString();
    }
